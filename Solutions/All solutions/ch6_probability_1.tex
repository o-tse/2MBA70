...
\item Note that by assumption we have that
\[
	X_\# \bbP((-\infty, t]) := F_X(t) = F_Y(t) := Y_\# \bbP((-\infty, t]),
\]
holds for all intervals $(-\infty, t]$. Moreover, these intervals generate the Borel \sigalg/, see Proposition 2.7. Therefore the idea is to use Theorem 2.15 to conclude that $X_\# \bbP = Y_\# \bbP$.

For this we need to check two properties for the generator set $\cA = \{(-\infty, t] \, : \, t \in \bbR\}$.
\begin{enumerate}[label=\arabic*.]
\item Suppose $A = (-\infty, t]$ and $B = (-\infty, s]$ with $t \le s$. Then $A \cap B = (-\infty, t] = A \in \cA$.
\item Take $A_n = (\infty,n]$. Then $\bigcup_{n \in \bbN} A_n = \bbR$.
\end{enumerate}

We thus conclude that $\cA$ satisfies the requirements of Theorem 2.15 and thus $X_\# \bbP = Y_\# \bbP$ on the entire Borel \sigalg/.
\item First note that there must exist a measurable set $N \in \cB_\bbR$ such that $X_\# \bbP(N) = 0$. If not, then using the sequence of disjoint sets $A_n = [n,n+1)$ we get that
\[
	X_\# \bbP(\bbR) \ge X_\# \bbP(\bigcup_{n \in \bbN} A_n) = \sum_{n \in \bbN} X_\#\bbP(A_n) = \infty,
\]
contradicting $X_\# \bbP(\bbR) = 1$.

Define the function $Y : \Omega \to \bbR$ as follows:
\[
	Y(\omega) = \begin{cases}
		X(\omega) &\text{if } \omega \in \bbR \setminus N,\\
		X(\omega) + 2 &\text{if } \omega \in N.
	\end{cases}
\]
Then clearly $Y \ne X$ as functions. However, for any measurable set $A$, writing $A \Delta N := \Omega \setminus (A \cap N)$, we have
\begin{align*}
	X_\# \bbP(A) &= \bbP(X^{-1}(A)) \\
	&= \bbP(X^{-1}(A\cap N) \cup X^{-1}(A \Delta N)) \\
	&= \bbP(X^{-1}(A \cap N)) + \bbP(X^{-1}(A \Delta N))\\
	&= 0 + \bbP(\{\omega \in \Omega \, : \, X(\omega) \in A \Delta N\}) \\
	&= 0 + \bbP(\{\omega \in \Omega \, : \, Y(\omega) \in A \Delta N\}) \\
	&= \bbP(Y^{-1}(A \cap N)) + \bbP(Y^{-1}(A \Delta N))\\
	&= \bbP(Y^{-1}(A)) = Y_\# \bbP(A),
\end{align*}
...
\textbf{Problem 6.2}
\begin{enumerate}[label=(\alph*)]
\item The function $U(t)$ is measurable as it is a product of an indicator and a continuous function.
\item By definition of $U$ it follows that
\[
...
\textbf{Problem 6.8} \rubric{Total points: 8}
...
\begin{tabular}{p{0.8\linewidth}p{0.15\linewidth}}
\hline
Equality $\nu = X_\#\bbP$ on $(-\infty,t]$ & 1pt\\
Sets $(-\infty, t]$ generate $\cB_\bbR$ and satisfy conditions Theorem 2.15 & 1pt\\
Use Theorem 2.15 to conclude & 1pt\\
\hline
\end{tabular}
...
\begin{tabular}{p{0.8\linewidth}p{0.15\linewidth}}
\hline
Correct computation using definition $\nu$ and linearity integral & 2pt\\
\hline
\end{tabular}

}

Let $g = \sum_{i = 1}^N a_i \mathbbm{1}_{A_i}$ be a simple function. Then by definition of $\nu$ and linearity of the integral we get
\begin{align*}
...
\begin{tabular}{p{0.8\linewidth}p{0.15\linewidth}}
\hline
Correct application of MCT twice using (b) & 2pt\\
\hline
\end{tabular}

} 

By using (b) and monotone convergence twice we get
\begin{align*}
...
\begin{tabular}{p{0.8\linewidth}p{0.15\linewidth}}
\hline
Conclude using (a), (c) and change of variables & 1pt\\
\hline
\end{tabular}

}

Using the change of variables result (Proposition 4.14) (a) and (c) we get
\[
