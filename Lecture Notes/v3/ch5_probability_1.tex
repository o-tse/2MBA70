...
\begin{proof}[Old proof presented in the lecture (incomplete)]
...
The following result shows that this is indeed possible. Moreover, it provides a first nice usage of the Lebesgue measure.
...
\begin{proof}
See Problem~\ref{prb:construction_uniform_rv}
...
\subsection{Continuous random variables}\label{ssec:contrinuous_rv}
...
\begin{problem}\label{prb:construction_uniform_rv}
In this problem we will prove Lemma~\ref{lem:uniform_random_variable}. 
\begin{enumerate}[label=(\alph*)]
\item Prove that the function $U$ from the lemma is measurable.
\item Compute the preimage $U^{-1}((-\infty, t])$.
\item Use Theorem 2.19 to conclude the proof.
\end{enumerate}
\end{problem}
