...
It turns out that such a measure exists. This fundamental measure is called the \emph{Lebesgue measure} and, in addition to the property mentioned above, has several other strong features.
...
We end this section by looking at some important properties of measures.
...
The proof of this proposition is similar to that of Proposition~\ref{prop:continuity_measure_below} and is left as an exercise, see Problem~\ref{prb:proof_continuity_above}.
...
Let $(\Omega_i, \cF_i)$, for $i \in I$ be measurable spaces and $(f_i)_{i \in I}$ be a family of functions $f_i : \Omega \to \Omega_i$. Then the smallest \sigalg/ that makes all $f_i$ simultaneously measurable is
\[
...
\begin{proof}
We will show that any \sigalg/ that makes each $f_i$ measurable much contain $\sigma(f_i \, : \, i \in I)$. So let
$\cF$ be such a \sigalg/. Then in particular, for any $i\in I$ and $B \in \cF_i$ we have that $f_i^{-1}(B) \in \cF$. This implies that
\[
...
\subsection{Measurable functions on the real line}
...
Prove Proposition~\ref{prop:continuity_measure_above}.
\end{problem}
