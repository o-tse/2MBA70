...
We have seen in Section~\ref{ch:integrable} that the set of $\mu$-integrable functions form a vector space (over $\bbR$). For $p \in [0,+\infty)$, we define the vector space $\bbL^p$ of integrable functions $f$ for which 
\[
	\| f \|_{p} < +\infty.
\]
