...
\item $\mu_n \ast g \to \mu \ast g$ for all continuous bounded function $g : \bbR \to \bbR$ that are zero outside an interval $[-K,K]$ for some $K > 0$.
...
\textbf{1 $\iff$ 3:} The implication $1 \Rightarrow 3$ is trivial. So assume that $\mu_n \ast g \to \mu \ast g$ holds for all continuous bounded function that are zero outside some compact interval $[-K,K]$ and let $f : \bbR \to \bbR$ be a continuous bounded function with $\|f(x)\| \le M$ for all $x \in \bbR$. We will show that for any $\varepsilon > 0$
\[
	\|\mu_n \ast f - \mu \ast f\| \to \varepsilon,
\]
...
Let us now consider the measures $\mu_n \ast f$ and $\mu_n \ast fg$. Then we have that
\begin{align*}
	\|\mu_n\ast f(A) - \mu_n \ast fg(A)\|
	&= \|\int_A f \, \dd \mu_n - \int_A fg \, \dd \mu_n\|\\
	&= \|\int_A f (1-g) \, \dd \mu_n\| \le M \int_A (1-g) \, \dd \mu_n \\
	&\le M \int_\bbR (1-g) \, \dd \mu_n = M(1-\int_\bbR g \, \dd \mu_n)
\end{align*}
Since the later term converges to $\int_\bbR g \, \dd \mu$ by~\eqref{eq:conergence_distribution_1} we get that
\begin{align*}
	\limsup_{n \to \infty} \|\mu_n\ast f(A) - \mu_n \ast fg(A)\| &\le M(1- \int_\bbR g \, \dd \mu)\\
	&\le M \int_\bbR (1-g) \, \dd \mu \le M \mu(\bbR \setminus [-\alpha,\alpha]) < \frac{\varepsilon}{2}.
\end{align*}
The same conclusion holds true for $\|\mu\ast f(A) - \mu \ast fg(A)\|$.

If we now write
\begin{align*}
	\|\mu_n \ast f(A) - \mu \ast f(A)\| &\le \|\mu_n\ast f(A) - \mu_n \ast fg(A)\| + \|\mu_\ast f(A) - \mu \ast fg(A)\|\\ &\hspace{10pt}+ \|\mu_n \ast fg(A) - \mu \ast fg(A)\|
\end{align*}
