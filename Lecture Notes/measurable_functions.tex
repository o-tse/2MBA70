...
\begin{problem}
	Explicitly construct two independent random variables $X$ and $Y$ such that $X$ is \emph{Poisson} distributed with parameter $\lambda>0$, and $Y$ is \emph{Cauchy} distributed with parameter $\gamma>0$. More precisely define a probability space $(\Omega, \cF, \bbP)$ and $(\cF,\cB_\bbR)$-measurable functions $X\colon \Omega\to \bbR$ and $Y\colon \Omega\to\bbR$ such that
	\begin{align*}
		\bbP(X=n) &= \frac{e^{-\lambda}\lambda^n}{n!},\qquad \forall\,n\in\bbN,\\
		\bbP(Y\le z) &= \frac{1}{\pi}\arctan\left(\frac{z}{\gamma}\right) + \frac{1}{2},\qquad \forall\,x\in\bbR.
	\end{align*}
It is recommended that you go through the following steps:
\begin{enumerate}[label=(\alph*)]
	\item Define $(\Omega, \cF, \bbP)$ and the functions $X\colon \Omega\to \bbR$ and $Y\colon \Omega\to\bbR$.
	\item Show that $X$ and $Y$ are $(\cF,\cB_\bbR)$-measurable.
	\item Show that the equalities above hold.
\end{enumerate}
\end{problem}
