...
This assessment consists of 4 problems, reflecting results stated in the lecture notes, worth a total of 27 points. For solving them you are of course \textbf{not} allowed to simply use the corresponding result from the lecture notes. You are allowed to use any other results from the course and lecture notes and any results derived in this assessment. 

\vspace*{20pt}

\textbf{Problem 1} (7 Points)
...
\textbf{CONTINUES ON NEXT PAGE}

\pagebreak



\textbf{Problem 4} (9 Points)
...
\begin{enumerate}[label={(\alph*)}]
\item Suppose that $X, Y$ are $\cH$-measureable and $\int_B X d \bbP = \int_B Y \bbP$ holds for all $B \in \cH$. Prove that $X = Y$ $\bbP$-almost everywhere, i.e. $\int_\Omega |X- Y| d \bbP = 0$.
\item If $X$ is $\cH$-measurable, prove that $\bbE[X | \cH] = X$ holds $\bbP$-almost everywhere.
