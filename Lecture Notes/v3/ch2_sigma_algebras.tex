...
Next we will discuss one of the most important \sigalgs/ that is constructed from a generator set.
...
Let $d \ge 1$. The Borel \sigalg/ $\cB_{\bbR^d}$ on $\bbR^d$ is the \sigalg/ generated by any of the following family of sets:
\begin{enumerate}[label={(\alph*)}]
...
%any of the following family of sets, 
...
In the rest of these lecture notes we will often perform analysis of integral or functions under the assumption that they satisfy a certain property, except on a set of measure zero. We make this precise with the following definition.

\begin{definition}[Almost everywhere]\label{def:almost_everywhere}
Let $(\Omega, \cF, \mu)$ be a measure space. We say that a property hold \emph{$\mu$-almost everywhere} if the collection of $\omega \in \Omega$ for which it does not hold is a measurable set $A \in \cF$ with $\mu(A) = 0$. 
\end{definition}
...
\pagebreak
\begin{theorem}[Equivalent definition of a measure]\label{thm:measure_equivalent_def}
...
\item there exists a sequence $(A_i)_{i \in \bbN}$ with $A_i \in \cA$ and $\Omega = \bigcup_{i \in \bbN} A_i$.
\end{enumerate}
...
In Section~\ref{ssec:construction_sigalgs} we encountered a general and useful way to construct \sigalgs/ from smaller sets. Here we will take a similar approach to constructing measures. The starting point here is a `'measure-like" function, called a \emph{pre-measure} defined something that is not exactly a \sigalg/, called a \emph{semi-algebra}. We will start with the definition of the latter.
...
\item For any $A \in \mathcal{S}$, there exists a finite family of mutually disjoint sets $C_1, \dots, C_m \in \mathcal{S}$ such that $\Omega \setminus A = \bigcup_{i = 1}^m C_i$.
\end{enumerate}
\end{definition}

An example of a semi-algebra for a finite set $\Omega$ is the set of singletons $\mathcal{S} = \{\{\omega\} \in \Omega\} \cup \{\emptyset, \Omega\}$. A more important example in the case of $\bbR$ is the collection of semi-open intervals
\begin{equation}\label{def:right_closed_intervals}
	\mathcal{S} = \{\emptyset\} \cup \{(a,b] \, : \, a\le b, a, b \in \bbR\} \cup \{(a,+\infty) \, : \, a \in \bbR\} \cup \{(-\infty, a] \, : \, a \in \bbR \} \cup \{\bbR\},
\end{equation}
...
The utility of this somewhat seemingly abstract notion of a semi-algebra is that it serves as the starting point to construct measures from a pre-measure. This is a set function $\mu : \mathcal{S} \to [0,+\infty]$  that satisfies the two properties of a measure as given in Definition~\ref{def:measure}. Note that the only difference with a measure is that the domain of $\mu$ is a semi-algebra $\mathcal{S}$ and not a \sigalg/. 

\begin{remark}
It is important to note that the result of Theorem~\ref{thm:measure_equivalent_def} remains true in the case of pre-measures on a semi-algebra. See also Problem~\ref{prb:alternative_def_premeasure}.
\end{remark}

The key result of this section is that it is possible to extend a pre-measure $\mu$ to a proper, and often unique, measure on the generated \sigalg/ $\sigma(\mathcal{S})$. In addition to this, the measure will be defined on a slightly larger \sigalg/ that has some nice properties. To formally state this we need to introduce the concept of \emph{null sets} and \emph{complete measure spaces}.
...
With this setup, we can now state the main result that allows us to extend pre-measures to complete measures.
...
%The interested student can look at [REF Appendix] for the details.
...
%The interested student is referred to the Appendix, where we provide the full details.  
...
%This \sigalg/ is called the \emph{Lebesgue \sigalg/} and functions that are $\cL^d$-measurable are called \emph{Lebesgue measurable}. 
...
The goal of this problem is to prove Proposition~\ref{prop:characterization_borel_sigalg}. We will do this in several stages. First we will show point 1, that the Borel \sigalg/ $\cB_\bbR$ is generated by either the family 
\[
	\cA_1 := \{(a_1, b_1) \times \dots \times (a_d, b_d), a_i < b_i \in \bbQ\},
\]
or 
\[
	\cA_1^\prime := \{(a_1, b_1) \times \dots \times (a_d, b_d), a_i < b_i\in \bbR\}.
\]
\begin{enumerate}[label=(\alph*)]
\item Prove that $\sigma(\cA_1) \subset \sigma(\cA_1^\prime) \subset \cB_\bbR$. [Hint: what is the relation between a set $(a_1,b_1) \times \dots \times (a_d, b_d)$ and an open set?]
...
\begin{enumerate}[label=(\alph*)]
\setcounter{enumi}{4}
\item Show that for any collection $a_i < b_i \in \bbR$
\[
	(a_1,b_1] \times \dots \times (a_d, b_d] = \bigcap_{j \in \bbN} (a_1, b_1+\frac{1}{j}] \times \dots \times (a_d, b_d+\frac{1}{j}].
\]
\item Show that for any collection $a_i < b_i \in \bbR$
\[
	(a_1,b_1) \times \dots \times (a_d, b_d) = \bigcup_{j \in \bbN} (a_1, b_1-\frac{1}{j}] \times \dots \times (a_d, b_d-\frac{1}{j}].
\]
\item Prove that $\cB_\bbR = \sigma(\cA_2^\prime) = \sigma(\cA_2)$, where 
\[
	\cA_2 = \{(a_1,b_1] \times \dots \times (a_d, b_d] \, : \, a_i < b_j \in \bbQ\},
\] 
and 
\[
	\cA_2^\prime = \{(a_1,b_1] \times \dots \times (a_d, b_d] \, : \, a_i < b_i \in \bbR\}.
\]
\end{enumerate}

This basically covers the full set of ideas to prove the rest of Proposition~\ref{prop:characterization_borel_sigalg}. We invite you to work these out yourself to practice with these kind of arguments. For this problem however we will ask you to explain the idea for the proofs.
\begin{enumerate}[label=(\alph*)]
\setcounter{enumi}{7}
\item Explain what changes in the proof of point 3 of Proposition~\ref{prop:characterization_borel_sigalg} from the proof of point 2 of this proposition outlined above.
\item Describe the proof strategy to get points 4-8 of Proposition~\ref{prop:characterization_borel_sigalg} using 1-4.
\end{enumerate}
\end{problem}
...
\begin{problem}\label{prb:alternative_def_premeasure}
Let $\cS$ be a semi-algebra on $\Omega$. Show that a set function $\mu : \cS \to [0,+\infty]$ is a \emph{pre-measure} if, and only if,
\begin{enumerate}
\item $\mu(\emptyset) = 0$,
\item $\mu(A \cup B) = \mu(A) + \mu(B)$, for any two disjoint sets $A, B \in \cS$, and
\item for any decreasing family $(A_i)_{i \in \bbN}$ of measurable sets such that $\mu(A_1) < \infty$ and $A_\infty := \bigcap_{i \in \bbN} A_i \in \cS$, it holds that
\[
	\mu(A_\infty) = \lim_{i \to \infty} \mu(A_i) \quad (= \inf_{i \in \bbN} \mu(A_i)).
\]
\end{enumerate}
\end{problem}
